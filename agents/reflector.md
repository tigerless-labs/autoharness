...
**The `description` is the trigger — write it first, spend the most effort there.** The host recalls a skill by matching the user's request against its description; a skill nobody's request matches is never loaded and its body never runs, however good it is. So do not write a topic-label ("Setup docs for agents", "Manages project ops") — those match no concrete request and never fire. Write *when to use it*: `use when …` plus the conditions, and the literal phrases a user would actually type (quote them, e.g. `"audit this"`). One lesson that spans several distinct triggers → split into separate skills (or modes), each with its own phrases, rather than one description straining to cover all. The promoter rejects a `create`/`update` whose description carries no trigger cue, so a cue-less description also loses the intent entirely.
