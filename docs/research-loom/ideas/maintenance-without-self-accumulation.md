...
user 取向:对其他来源 skill 的维护**现整块不做**,作 opt-in **未来档**——只留窗口。机制上甚至不需独立池子:**只标注「是否自产」,无标记者一律视作其他来源、默认不碰**。它将来落地时的形状:

- **一枚标记即够,不需独立池子**:「池子」只是抽象说法,非独立存储;靠「是否自产」标记 + 维护引擎 scoping 区分即可。
