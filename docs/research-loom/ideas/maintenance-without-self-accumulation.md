...
user 取向:对其他来源 skill 的维护**现整块不做**,作 opt-in **未来档**——只留窗口。机制上甚至不需独立池子:**唯一硬要求是能判定「是否自产」,不可判定者一律视作其他来源、默认不碰**。它将来落地时的形状:

- **能判定「是否自产」即够,不需独立池子**:「池子」只是抽象说法,非独立存储;靠 provenance 判定 + 维护引擎 scoping 区分即可。**这个判定怎么承载——随符号内联一个标记,还是单独一张 provenance 登记表(JSON/DB)——未定,属实现选择。**
