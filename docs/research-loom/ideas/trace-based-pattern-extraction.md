...
**但只借前端**：ECC 的 trace 提取有硬伤，恰是 autoharness 要补的地方——只看 tool trace、**读不到用户原话**（纠正全靠从重编辑/回退反推）；窗口截断（>500 行即盲区）；「模式/重复/冲突」全交给模型 gestalt，**无算法定义、不可复现**。所以 trace 只能当**候选来源**，不能当准入裁决；裁决归 [结构化把关](structural-gate-no-oracle.md)。

## 关联

是 [结构化把关，不要 held-out 分数](structural-gate-no-oracle.md) 的上游来源（trace 供料 → 结构门把关）。提取出的「固定工作流」模式即潜在的**顺序边**，与 DAG 边蒸馏同源。
