...
reflector ──stage_skill──► intent（不落盘）
   promoter：
     ...
     ② 校验（内存）：linter（见下），此刻同握 old=live + new=成型结果
     ③ pass → 写 temp（live 同目录 `.SKILL.md.tmp`）+ fsync + os.replace 原子改名 → live
        reject → 什么都没写
```
...
逐 intent 跑，产 verdict `{pass / reject + findings}`，**全确定性**（快、精确、不可注入）。查五类：

- **安全**：`skills_guard`（见下）。
- **结构（#416）**：frontmatter 可解析 + 必填（name / description）；引用的**文件存在**；`.py` 过语法解析；无断 symlink。
...
- **create / update** → 写 temp 同目录 + `os.replace` 原子 rename 进**对应层**（create → intent 的 `level`；update → 定位到的 live 层）+ 标 `created_by:agent`（sidecar，见下）+ 把 **intent 自带的 LED 条目** append 进真 LED（append-only、只记既成事实）。
...
- **绕不过**：reflector 工具只有 `Read / Grep / stage_skill`，stage_skill 只记 intent → **模型结构上无法 land、跳不过闸、碰不到 live**；最大效果 = 塞一条 intent。
...
- **改（更干净）**：Hermes 需 ContextVar，是因为**模型直接写盘、得探测"我在不在 review fork"**；我们的 **promoter 是 live 唯一写者、只从 intent 成型落盘**——**按构造就知道是自产**，无需 origin 探测，确定性盖 `created_by:agent`。
...
- **linter 五类**（安全 `skills_guard` + #416 结构 + LED 有 + 内容完整 + global repo-agnostic），跑在内存成型结果上，**全确定性、不调 LLM**。
...
- created_by 走 sidecar；promoter 按构造盖，不用 origin 探测。
